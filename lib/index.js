...
class IoTAgent {
   ...
   */
  terminate() {
    logger.debug("Terminating the IoT agent...", TAG);
    Promise.all([
      this.messenger.producer.disconnect(),
      this.messenger.consumer.disconnect(),
      this.messenger._tenancyConsumer.disconnect(),
    ]).then(() => {
      logger.debug("IoT agent terminated.", TAG);
    }).catch((error) => {
      logger.error(`... failed to terminate the IoT agent. Error: ${error.message}`, TAG);
